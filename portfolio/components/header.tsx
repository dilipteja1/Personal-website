...
export default function Header(){
    return <header  className="z-[999] relative">
        ...
        <nav className="flex fixed left-1/2 h-12 -translate-x-1/2 py-2 sm:top-6">
            <motion.ul className="flex flex-wrap items-center justify-center gap-3 sm:gap-10 sm:flex-nowrap text-gray-500"
            initial={{y:-100, opacity:0}}
            animate={{y:0, opacity:1}}>
                {links.map( link => (
                    <li className="flex items-center justify-center hover:text-gray-950 transition" 
                    key = {link.hash}>
